...
static cubs_index_t cubs_spline_mem_calc(
    cubs_index_t count,
...
struct spline_matrix *cubs_create_matrix(
    struct spline_matrix *matrix, //Must be pre-allocated
	cubs_index_t size
)
{
    cubs_index_t i, div;
...
)
{
    ...
    cubs_index_t t;
    for (t = 0; t <= terms; ++t){
        value += matrix->inv_mat[matrix->length - (pos + 2 * t)];
    }
    return value * ((i + j) % 2 ? -1.0 : 1.0);
}

struct spline_matrix *cubs_cache_add(
    cubs_cache_t cache,
    cubs_index_t length
)
{
...
)
{
    ...
    for ( i = 0; i < m; ++i){
        cubs_number_t *value = C2 + i * spline->dimensions;
        for ( j = 0; j < n; ++j){
    ...
    for ( i = 0; i < n - 1; ++i ){
        for ( j = 0; j < spline->dimensions; ++j ){
            ...
            const cubs_index_t first = i;
            const cubs_index_t second = i+1;
            const cubs_index_t first_value = spline->point_data[first * spline->dimensions ];
            const cubs_index_t second_value = spline->point_data[second * spline->dimensions ];
...
static void cubs_evaluate_params(
	cubs_spline_t spline,
	cubs_number_t *t IN_OUT,
	cubs_index_t *idx OUT
)
{
    ...
    *t -= spline->minimum;
    *t -= (cubs_number_t)*idx / spline->func_size * range;
}
